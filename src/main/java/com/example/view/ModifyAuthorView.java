...
public class ModifyAuthorView extends JFrame implements AuthorView {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ModifyAuthorView frame = new ModifyAuthorView();
	...
	public ModifyAuthorView() {
		...
		lblName.setBounds(594, 204, 63, 17);
		contentPane.add(lblName);
		
		textName = new JTextField();
		textName.setBounds(667, 205, 157, 19);
		contentPane.add(textName);
		textName.setColumns(10);
		
		JTextPane txtpnParaModificarO = new JTextPane();
		txtpnParaModificarO.setFont(new Font("Tahoma", Font.PLAIN, 14));
		txtpnParaModificarO.setText("Para modificar o eliminar libros, edítelos directamente desde las vistas para los libros.");
		txtpnParaModificarO.setEditable(false);
		txtpnParaModificarO.setBounds(594, 398, 239, 60);
		contentPane.add(txtpnParaModificarO);
		...
		btnModify.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(table.getSelectedRow() != -1) {
					// Comprobamos que el campo esté relleno.
					if(!textName.getText().isEmpty()) {
						...
						showresult(miController.modifyAuthor(selectedAuthorId, textName.getText()));
						
					} else {
						showresult("Debe escribir el nuevo nombre del autor.");
					}
				} else {
				...
				dtm.setRowCount(0);
				authors.clear();
				miController.showModifyAuthors();
				showAuthors();
				
				// Por último, vaciamos el campo de la vista.
				textName.setText("");
			}
		});
		btnModify.setBounds(686, 268, 122, 21);
		contentPane.add(btnModify);
		
		textResults = new JTextPane();
		textResults.setBackground(new Color(0, 128, 192));
		textResults.setBounds(60, 491, 467, 53);
